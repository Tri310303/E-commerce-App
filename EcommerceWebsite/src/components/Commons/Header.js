        ...
        <Navbar expand="lg" className="bg-body-tertiary">
            <Container>
                ...
                <Nav className="me-auto">
                    ...
                    <Link to="/store" className="nav-link text-danger">Cửa hàng</Link>
                    <Link to="/cart" className="nav-link">&#128722; <Badge bg="danger">{value}</Badge></Link>
                    {user !== null?<>
                        <Link to="/" className="nav-link text-danger">Hi,{user.username}</Link>
                        <Link onClick={() => dispatch({"type": "logout"})} className="nav-link text-danger">Đăng xuất</Link>
                        <Link to="/chart" className="nav-link text-danger">Thống kê</Link>
                    </>:<>
